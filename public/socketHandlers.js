...
const writeUserToBackend = (userId) => {
    console.log(
        `[main.js/writeUserToBackend] Sending user to backend: ${userId}`,
    );
    fetch("/writeUser", {
        method: "POST",
        headers: {
        ...
        .then((data) => {
            console.log(
                `[main.js/writeUserToBackend] Response from backend: ${data}`,
            );
        })
        .catch((error) => {
            console.error(
                `[main.js/writeUserToBackend] Error sending user to backend: ${error}`,
            );
        });
};
...
const eventListeners = () => {
    ...
    if (!userId) {
        ...
        writeUserToBackend(userId); // Write new user to backend
    } else {
        console.log(
